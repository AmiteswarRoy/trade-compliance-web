...
* [cssnext](http://cssnext.iox)
* [cssmodules](https://github.com/css-modules/css-modules)
* [purifyCSS](https://github.com/purifycss/purifycss)

## TL;DR

Use with `nodejs@4.3.1`, clone the repo,

* on mac `npm run install-deps` and `npm run dev`.
* on windows `npm run install-deps-win` and `npm run dev`.
...
Wrap you async actions into promises, send them to `altResolver` with `altResolver.resolve(xxx)`.
...
Use the same logic as localized string, declare the localized routes into `app/routes.js` and into your `i18n/{lang}` file.
...
* `$ git clone -o upstream https://github.dowjones.net/factiva-core/Isomorphic-ES7-Boilerplate.git app`
* `$ cd app && npm run install-deps`
